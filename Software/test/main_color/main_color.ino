...
///////ボタン関連
const int buttonOn_Pin = 46;  // ラインセンサをONにするボタン
const int buttonOff_Pin = 47; // ラインセンサをOFFにするボタン
bool game_flag = 0;  //ある角度以上になったらゲーム開始
int game_start = 0; //ボタンが押されたら1に


///////////definiton_motordriver/////////////
const int dir[] = {7,11, 6, 10}; //モーター0～3のpwmとdirの配列
const int pwm[] = {5,9, 4, 8};
int move_agle = 0;
...
double Line_Read() {
 ...
 for (int i = 0; i < 4; i++) {
  for (int j = 0; j < 4; j++) {
   double value = 1023 - analogRead(line[i][j]) - line_zero[i][j];
   print[(i * 4) + j] = value;
