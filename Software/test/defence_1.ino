...
const float pai = 3.14159265; // "pai" ではなく "pi" では？
int sp;
float gryo_val;
...
void loop() {
  ...
  while (game_flag == 1) {
    ...
    if (count_line > 0) { //前のラインセンサが反応したら横のラインも前として使用できるようにカウントを設定しました
        count_line++;
      if (count_line >= 50) {  // 10回カウントしたらリセット
            front_line = 0;
            count_line = 0;
        }
    }
    ...
    if (line_flag == 0) {
      ...
      if (ir_flag == 1) {
        angle_move = atan2(lengthx, lengthy) * 180 / pai + 180;
      //Run(angle_move,sp);
      //if (angle_move >= 0 && angle_move <= 25 /*&& line != 7*/) {
      //Cal_power(0, sp, gryo_val);
      //} else if (angle_move >= 335 && angle_move <= 360 /*&& line != 7*/) {
      //Cal_power(0, sp, gryo_val);
      } else if (angle_move > 25 && angle_move < 50 /*&& line != 6*/) {
      Cal_power(90, sp, gryo_val);
      } else if (angle_move > 310 && angle_move < 335 /*&& line != 4*/) {
      Cal_power(270, sp, gryo_val);
      } else if (angle_move >= 50 && angle_move < 80 /*&& line != 6*/) {
      Cal_power(90, sp, gryo_val);
      } else if (angle_move > 280 && angle_move <= 310 /*&& line != 4*/) {
      Cal_power(270, sp, gryo_val);
      } else if (angle_move <= 105 && angle_move >= 80 /*&& line != 7*/) {
      Cal_power(90, sp, gryo_val);
      } else if (angle_move <= 280 && angle_move >= 255 /*&& line != 7*/) {
      Cal_power(270, sp, gryo_val);
      //} else if (angle_move <= 180 && angle_move > 150 /*&& line != 7*/) {
      //Cal_power(0, sp, gryo_val);
      //} else if (angle_move > 180 && angle_move < 210 /*&& line != 6*/) {
      // Cal_power(0, sp, gryo_val);
      //} else if (angle_move > 105 && angle_move < 150 /*&& line != 7*/) {
      //  Cal_power(0, sp, gryo_val);
      //} else if (angle_move > 210 && angle_move < 255 /*&& line != 7*/) {
      //  Cal_power(0, sp, gryo_val);
      } else {
        Cal_power(angle_move, sp, gryo_val);
        //   //Run(0,sp);
      }
        int move_angle = 0;
