...
# Function to dynamically adjust thresholds based on lighting
def adjust_thresholds():
    global blue_threshold, green_threshold
    # Example: Adjust thresholds based on average brightness
    avg_brightness = sensor.get_rgb_gain_db()[0]
    blue_threshold = (24, 79 + int(avg_brightness), 81, -69, -115, -18)
    green_threshold = (62, 84 + int(avg_brightness), -28, -69, 75, 27)

# Function to filter blobs based on aspect ratio
def filter_blobs(blobs, min_aspect_ratio=0.5, max_aspect_ratio=2.0):
    return [b for b in blobs if min_aspect_ratio <= b.w() / b.h() <= max_aspect_ratio]

while True:
    adjust_thresholds()  # Adjust thresholds dynamically
    ...
    green_blobs = filter_blobs(green_blobs)  # Filter blobs
    ...
    if largest_green_blob:
        ...
        img.draw_rectangle(largest_green_blob.rect(), color=(0, 255, 0))  # Debug visualization
        img.draw_cross(largest_green_blob.cx(), largest_green_blob.cy(), color=(0, 255, 0))

        # 青色のブロブを取得
        blue_blobs = img.find_blobs([blue_threshold], pixels_threshold=50, area_threshold=50, merge=True)
        blue_blobs = filter_blobs(blue_blobs)  # Filter blobs
        ...
        if closest_blue_blob:
            ...
            img.draw_rectangle(closest_blue_blob.rect(), color=(0, 0, 255))  # Debug visualization
            img.draw_cross(closest_blue_blob.cx(), closest_blue_blob.cy(), color=(0, 0, 255))
