...
/////////////////////////////////////////////////////////
#define normal_speed 90  //移動速度 max speed
#define gryo_p 1.1
...
void ir_uart_recv(void) {
  ...
  if (buf[0] != 255 && buf[1] != 255 && buf[2] != 255 && buf[3] != 
255) {
  ...
  front_ir = analogRead(A0); // A0 ピンからデータを読み取り、front_ir に保存
      ...
      print[31] = front_ir;
  if(ir_angle >0 && ir_angle < 30 && ir_dist < 45){
    if(front_ir < 990 && front_ir > 975){
      ir_angle =0;
    }
...
void loop() {
  ...
  while (game_flag == 1) {
    ...
    if (count_line > 0) { //前のラインセンサが反応したら横のラインも前として使用できるようにカウントを設定しました
        count_line++;
      if (count_line >= 50) {  // 10 回カウントしたらリセット
            front_line = 0;
            count_line = 0;
        }
    }
      ...
      if (ir_flag == 1) {
        ...
        if (ir_dist < 60) {
            ...
            if (ir_angle >= 180 && ir_angle < 330) {
              ...
              move_invert_angle = (ir_invert_angle + 
constrain(ir_invert_angle * circ_exp * CIRC_WEIGHT, -90, 90));
              move_angle = 360 + move_invert_angle;
              if (move_angle > 270  && move_angle < 330){
                speed = 80;
              }
              if ( move_angle > 330 && front_ir > 950 && front_ir <
970){
                speed = 90;
              ...
              float circ_exp = pow(CIRC_BASE, ir_dist);  // 回り込みのための計算式．これがどうして導かれたかは謎．誰ももう覚えてない．
              // 計算で得られた角度が不適切な場合があるので，移動する方向を制限する．
              move_angle = ir_angle + constrain(ir_angle * 
CIRC_WEIGHT, -90, 90);
              if (move_angle > 20  && move_angle < 45){
                speed = 80;
              }
              if ( move_angle < 20 && front_ir > 970 && front_ir < 
990){
                speed = 90;
  ...
  if (game_start == 1) {
    if (90 <= gryo_val && gryo_val <= 200 || -90 <= gryo_val && 
gryo_val <= -200) {
      Cal_power(0, 0, -30);
    } else {
      game_start = 0;
