...
void camera() {
  ...
  if (goal_flag == 1 && millis() - lastGoalFlagTime > goalFlagDuration) {
    goal_flag = 0;
  }
...
double Cal_power(double degree, double speed, double gyro_value) {
  static double position_x = 0.0;  // ロボットの現在のX座標
  static double position_y = 0.0;  // ロボットの現在のY座標
  static unsigned long last_update_time = millis();  // 前回の更新時刻
  
  // first_flag が 0 の場合、座標を初期化
if (first_flag == 0 && posi_flag == 1) {
    position_x = 0.0;
    position_y = 0.0;
    first_flag = 1;  // 初期化が完了したらフラグを 1 に設定
  ...
  // X, Y座標を更新
  position_x += distance * cos(radian);
  position_y += distance * sin(radian);
...
void loop() {
  ...
  while (game_flag == 1) {
    ...
    if (goal_height1 < 115 && goal_height1 > 70) {
